...
int main() {
    ...
    for (const auto& contour : contours3) {
        ...
        if (area > 1000) {
            ...
            // Draw centroid
            cv::circle(image1, centroid, 5, cv::Scalar(0, 0, 255), -1);
             // Print centroid coordinates
            std::cout << "Centroid of blue block of the indoor picture: (" << centroid.x << ", " << centroid.y << ")" << std::endl;
        }
    }
    ...
    for (const auto& contour : contours1) {
        ...
        if (area > 10000 ){
            ...
            cv::circle(image1, centroid, 5, cv::Scalar(0, 0, 255), -1);
            // Print centroid coordinates
            std::cout << "Centroid of red block of the indoor picture: (" << centroid.x << ", " << centroid.y << ")" << std::endl;
        }
    }
        ...
        for (const auto& contour : contours4) {
            ...
            if (area > 1000) {
                ...
                cv::Point centroid(moments.m10 / moments.m00, moments.m01 / moments.m00);
                // Draw centroid
                cv::circle(image2, centroid, 5, cv::Scalar(0, 0, 255), -1);
                // Print centroid coordinates
                std::cout << "Centroid of blue block of the outdoor picture: (" << centroid.x << ", " << centroid.y << ")" << std::endl;
            }
        }
    ...
    for (const auto& contour : contours2) {
        ...
        if (area > 10000 ){
            ...
            cv::circle(image2, centroid, 5, cv::Scalar(0, 0, 255), -1);
            // Print centroid coordinates
            std::cout << "Centroid of red block of the outdoor picture: (" << centroid.x << ", " << centroid.y << ")" << std::endl;
        }
    }
